...
pub contract MPoweredNFT : NonFungibleToken, LicensedNFT {
    ...
    pub resource Minter {
		...
		pub fun mintEditions(creator: &MPoweredNFT.Collection{MPoweredNFTCollectionPublic}, 
		name: String, description: String, image: String, unlockableContent: String, setId: UInt64, metadata: metadata, editionSize: UInt16, royalties: [LicensedNFT.Royalty]) {
			pre {
				self.publicMinting: "Minting is currently closed by the Administrator!"
